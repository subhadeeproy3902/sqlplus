...
export default function SQLTerminal() {
  ...
  const [authStep, setAuthStep] = useState<"ask" | "username" | "password">(
    "ask"
  );
  ...
  useEffect(() => {
    if (inputRef.current) {
      ...
      if (!authState.isAuthenticated) {
        ...
        // Authenticated: auto-grow for SQL input
  ...
  useEffect(() => {
    if (!authState.isAuthenticated && authStep === "ask") {
      ...
      if (
        !lastLine ||
        lastLine.content !==
          "Welcome! Type LOGIN to sign in or REGISTER to create an account."
      ) {
        addLine(
          "output",
          "Welcome! Type LOGIN to sign in or REGISTER to create an account."
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (
      e.ctrlKey &&
      (e.key.toLowerCase() === "c" || e.key.toLowerCase() === "с")
    ) {
      e.preventDefault();
      ...
      const displayValue = isPasswordInput
    ...
    if (!authState.isAuthenticated) {
      if (e.key === "Enter" && !e.shiftKey) {
        // Simple Enter submits
        e.preventDefault();
        handleCommand();
      } else if (e.key === "Tab" || (e.key === "Enter" && e.shiftKey)) {
        // Disable Tab and Shift+Enter for newlines
        e.preventDefault();
      }
        ...
        if (e.shiftKey) {
          ...
          if (trimmedInput.endsWith(";") || isSpecialCommand) {
            ...
            if (currentInput.trim()) {
              // Only add numbering if there's content
    ...
    if (e.key === "ArrowUp") {
      ...
      if (showingFullCommand && authState.isAuthenticated) {
        ...
        // Was at the first history item, now going "down" to empty
        setHistoryIndex(-1);
        setCurrentInput("");
        setShowingFullCommand(false);
        // Focus the input
  ...
  const handleCommand = async () => {
    ...
    // Mask if password.
    const displayInput = isPasswordInput
      ? "*".repeat(currentInput.length)
      : currentInput;
    ...
    if (inputRef.current) {
      // Reset height after command submission
      inputRef.current.style.height = "auto";
    }
  ...
  const handleLogin = async (username: string, password: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        addLine(
          "success",
          `Last Successful login time: ${new Date().toLocaleString()}`
        );
        addLine("output", "");
  ...
  const handleSQLCommand = async (input: string) => {
    ...
    if (
      cleanedInput &&
      !commandHistory.some((item) => item.fullCommand === cleanedInput)
    ) {
    ...
    if (commandToCheck === "show tables") {
      ...
      const currentUserName = (authState.username || "").replace(
        /[^a-zA-Z0-9_]/g,
        "_"
      );
      ...
      setLines((prevLines) => [
        ...prevLines,
        {
          ...
          content: "",
          isLoader: true,
          loaderMessage: "Getting your tables...",
        },
      ]);
    ...
    if (!cleanedInput.trim()) {
      // Check if cleanedInput is empty
      return;
    }
  ...
  const handleAICommand = async (input: string) => {
    ...
    if (!prompt) {
      addLine(
        "error",
        "Please provide a prompt after /ai command. Example: /ai show me all users"
      );
      addLine("output", "");
      return;
    }
    ...
    try {
      ...
      if (result.success && result.sqlCommands) {
        ...
        addLine(
          "error",
          `AI Error: ${result.error || "Failed to generate SQL"}`
        );
      }
    } catch (error) {
      setLines((prevLines) => prevLines.filter((line) => !line.isLoader));
  ...
  return (
    <div className="h-screen bg-white dark:bg-black text-black dark:text-white font-mono text-sm overflow-hidden flex flex-col relative">
      ...
      <div
        ref={terminalRef}
        className="flex-1 overflow-y-auto p-2 space-y-1 terminal-output"
      >
        {lines.map((line, index) => {
          let specialColor = "";
          if (
            line.type === "output" &&
            line.content.startsWith("🤖 Generating SQL for:")
          ) {
            specialColor = "text-blue-400 dark:text-blue-300";
          }
